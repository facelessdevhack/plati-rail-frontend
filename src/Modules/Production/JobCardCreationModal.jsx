...
const JobCardCreationModal = ({ visible, onCancel, onSuccess, selectedPlan = null }) => {
  ...
  const DEFAULT_PRESET = {
    ...
    isActive: true,
    steps: [
      { id: 1, name: 'Material Request', estimatedDuration: 2, estimatedDurationUnit: 'hours', stepOrder: 1, isRequired: true, icon: '📦' },
      { id: 2, name: 'Painting', estimatedDuration: 4, estimatedDurationUnit: 'hours', stepOrder: 2, isRequired: true, icon: '🎨' },
      { id: 3, name: 'Machining', estimatedDuration: 6, estimatedDurationUnit: 'hours', stepOrder: 3, isRequired: true, icon: '⚙️' },
      { id: 4, name: 'PVD Powder Coating', estimatedDuration: 3, estimatedDurationUnit: 'hours', stepOrder: 4, isRequired: true, icon: '🔧' },
      { id: 5, name: 'PVD Process', estimatedDuration: 5, estimatedDurationUnit: 'hours', stepOrder: 5, isRequired: true, icon: '⚡' },
      { id: 6, name: 'Milling', estimatedDuration: 4, estimatedDurationUnit: 'hours', stepOrder: 6, isRequired: true, icon: '🏭' },
      { id: 7, name: 'Acrylic Coating', estimatedDuration: 3, estimatedDurationUnit: 'hours', stepOrder: 7, isRequired: true, icon: '💧' },
      { id: 8, name: 'Lacquer Finish', estimatedDuration: 2, estimatedDurationUnit: 'hours', stepOrder: 8, isRequired: true, icon: '✨' },
      { id: 9, name: 'Packaging', estimatedDuration: 1, estimatedDurationUnit: 'hours', stepOrder: 9, isRequired: true, icon: '📋' },
      { id: 10, name: 'Quality Check', estimatedDuration: 2, estimatedDurationUnit: 'hours', stepOrder: 10, isRequired: true, icon: '🔍' },
      { id: 11, name: 'Dispatch', estimatedDuration: 1, estimatedDurationUnit: 'hours', stepOrder: 11, isRequired: true, icon: '🚚' }
    ]
  }
  ...
  useEffect(() => {
    if (allocationInfo.remainingQuantity > 0 && visible && currentStep === 1) {
      ...
      setTimeout(() => {
        ...
        console.log('Available remaining quantity:', allocationInfo.remainingQuantity)
        
        // Set the quantity if it's not set or if we just moved to this step
        if (!currentQuantity || currentQuantity !== allocationInfo.remainingQuantity) {
          form.setFieldsValue({
    ...
    if (selectedPlan && visible) {
      ...
      const allocInfo = {
        ...
        percentage: selectedPlan.quantity ? 
          Math.round(((tracking.totalJobCardQuantity || 0) / selectedPlan.quantity) * 100) : 0
      }
  ...
  const loadStepPresets = async () => {
    ...
    try {
      ...
      const formattedPresets = apiPresets.map(preset => ({
        ...
        description: preset.presetDescription || preset.description || 'No description',
  ...
  const loadAvailableSteps = async () => {
    ...
    try {
      ...
      const steps = result || []
      setAvailableSteps(steps.map(step => ({
        ...step,
    ...
    if (plan) {
      ...
      const allocInfo = {
        ...
        percentage: plan.quantity ? 
          Math.round(((tracking.totalJobCardQuantity || 0) / plan.quantity) * 100) : 0
      }
      setAllocationInfo(allocInfo)
      
      // Set quantity with a small delay to ensure the form field is ready
      setTimeout(() => {
        const quantityToSet = allocInfo.remainingQuantity > 0 ? allocInfo.remainingQuantity : 1
        form.setFieldsValue({
  ...
  const handlePresetSelection = async (presetId) => {
    ...
    try {
      ...
      if (Array.isArray(result)) {
        presetSteps = result
      } else if (result.data && result.data.steps && Array.isArray(result.data.steps)) {
      ...
      const formattedSteps = stepsCopy
        .sort((a, b) => (a.stepOrder || a.step_order || 0) - (b.stepOrder || b.step_order || 0))
        .map((step, index) => ({
          id: step.stepId || step.step_id || step.id || index + 1,
          name: step.stepName || step.step_name || step.name || 'Step ' + (index + 1),
          ...
          estimatedDuration: step.estimatedDuration || step.estimated_duration || 2,
          estimatedDurationUnit: step.estimatedDurationUnit || step.estimated_duration_unit || 'hours',
      ...
      notification.error({
        ...
        description: 'Please try selecting another preset or use the default workflow.'
      })
      // Fallback to default preset
  ...
  const handleQuantityChange = (value) => {
    const shouldShowReason = value && allocationInfo.remainingQuantity && 
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const prodPlanId = values.prodPlanId || selectedProductionPlan?.id || selectedPlan?.id
      ...
      title={
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <Title level={4} className="mb-0">Create Job Card</Title>
          ...
          <Space>
            ...
            {currentStep < wizardSteps.length - 1 ? (
              <Button 
                type="primary" 
                onClick={() => {
                  const fields = currentStep === 0 ? ['prodPlanId'] : 
                               currentStep === 1 ? ['quantity'] : []
                  
                  form.validateFields(fields)
                    .then((values) => {
                      console.log('Validated values for step', currentStep, ':', values)
          ...
          <div>
            {selectedPlan ? (
              ...
              <Form.Item
                ...
                rules={[{ required: true, message: 'Please select a production plan' }]}
              >
                <Select
                  ...
                  {productionPlans
                    ...
                    .map(plan => (
                      <Option 
                        key={plan.id} 
                        value={plan.id}
                        searchtext={`${plan.id} ${plan.sourceproductname || plan.alloyName} ${plan.targetproductname || plan.convertName}`}
                      >
                        <div className="py-2">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <Text strong>#{plan.id}</Text>
                              <Text>{plan.sourceproductname || plan.alloyName} → {plan.targetproductname || plan.convertName}</Text>
                              {plan.isUrgent && (
              ...
              <div className="mt-4">
                <Card 
                  title={
                    <div className="flex items-center justify-between">
                      ...
                      <Tag color={(selectedProductionPlan || selectedPlan)?.isUrgent || (selectedProductionPlan || selectedPlan)?.urgent ? 'red' : 'blue'}>
                ...
                >
                  <Row gutter={[16, 16]}>
                    <Col span={12}>
                      <div className="bg-gray-50 p-3 rounded">
                        <div className="text-xs text-gray-500 mb-1">Source Alloy</div>
                        <div className="font-medium text-base">
                          {console.log(selectedProductionPlan, "SELECTED")}
                          {(selectedProductionPlan || selectedPlan)?.sourceproductname || 'N/A'}
                        </div>
                      </div>
                    </Col>
                    <Col span={12}>
                      <div className="bg-gray-50 p-3 rounded">
                        <div className="text-xs text-gray-500 mb-1">Target Product</div>
                        <div className="font-medium text-base">
                          {(selectedProductionPlan || selectedPlan)?.targetproductname || 'N/A'}
                        </div>
                      </div>
                    </Col>
                    <Col span={8}>
                      <div className="bg-blue-50 p-3 rounded">
                        ...
                        <div className="font-bold text-xl text-blue-600">
                          {(selectedProductionPlan || selectedPlan)?.quantity?.toLocaleString() || 0}
                        </div>
                      </div>
                    </Col>
                    <Col span={8}>
                      <div className="bg-green-50 p-3 rounded">
                        ...
                        <Tag color={getStatusColor((selectedProductionPlan || selectedPlan)?.status)}>
                          {(selectedProductionPlan || selectedPlan)?.status || 'Pending'}
                        </Tag>
                      </div>
                    </Col>
                    <Col span={8}>
                      <div className="bg-purple-50 p-3 rounded">
                        ...
                        <Tag 
                          icon={(selectedProductionPlan || selectedPlan)?.isUrgent || (selectedProductionPlan || selectedPlan)?.urgent ? <FireOutlined /> : null}
                          color={(selectedProductionPlan || selectedPlan)?.isUrgent || (selectedProductionPlan || selectedPlan)?.urgent ? 'red' : 'default'}
                        >
                          {(selectedProductionPlan || selectedPlan)?.isUrgent || (selectedProductionPlan || selectedPlan)?.urgent ? 'URGENT' : 'Normal'}
                        </Tag>
                      </div>
                    </Col>
                  </Row>
                    ...
                    <Col span={12}>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">
                          {moment((selectedProductionPlan || selectedPlan)?.createdAt).format('MMM DD, YYYY HH:mm')}
                        </span>
                      </div>
                    </Col>
                    <Col span={12}>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">
                          {(selectedProductionPlan || selectedPlan)?.dealerName || 'Direct'}
                        </span>
                      </div>
                    </Col>
                  </Row>
                  ...
                  {(selectedProductionPlan || selectedPlan)?.notes && (
                    <Alert
                      message="Plan Notes"
                      description={(selectedProductionPlan || selectedPlan)?.notes}
                      type="info"
                      className="mt-3"
                    />
                  )}
                ...
                {(selectedProductionPlan?.quantityTracking || selectedPlan?.quantityTracking) && (
        ...
        {currentStep === 1 && (
          <div>
            ...
            <Card 
              className="mb-4"
              style={{
                background: allocationInfo.remainingQuantity === 0 ? 
                ...
                {allocationInfo.remainingQuantity > 0 && (
                  <Space className="-mt-4 mb-4">
                    <Button 
                      size="small" 
                      onClick={() => {
                        form.setFieldsValue({ quantity: Math.floor(allocationInfo.remainingQuantity / 4) })
                        handleQuantityChange(Math.floor(allocationInfo.remainingQuantity / 4))
                      }}
                    >
                      25%
                    </Button>
                    <Button 
                      size="small"
                      onClick={() => {
                        form.setFieldsValue({ quantity: Math.floor(allocationInfo.remainingQuantity / 2) })
                        handleQuantityChange(Math.floor(allocationInfo.remainingQuantity / 2))
                      }}
                    >
                      50%
                    </Button>
                    <Button 
                      size="small"
                      onClick={() => {
                        form.setFieldsValue({ quantity: Math.floor(allocationInfo.remainingQuantity * 0.75) })
                        handleQuantityChange(Math.floor(allocationInfo.remainingQuantity * 0.75))
                      }}
                    >
                      75%
                    </Button>
              ...
              <Alert
                ...
                description={
                  <Form.Item
                    ...
                    rules={[{ required: true, message: 'Reason is required for partial quantities' }]}
                    className="mb-0 mt-3"
                  >
                    <TextArea
            ...
            <Card size="small">
              <Descriptions column={2} bordered size="small">
                <Descriptions.Item label="Preset Name">
                  {(selectedProductionPlan || selectedPlan)?.workflowInfo?.presetName || (selectedProductionPlan || selectedPlan)?.presetName || 'Standard'}
                </Descriptions.Item>
                <Descriptions.Item label="Total Steps">
                  {(selectedProductionPlan || selectedPlan)?.workflowInfo?.totalSteps ?? (selectedProductionPlan || selectedPlan)?.stepCount ?? 11}
                </Descriptions.Item>
              </Descriptions>
            </Card>
          </div>
            ...
            <Card>
              <Descriptions column={2} bordered size="small">
                <Descriptions.Item label="Production Plan" span={2}>
                  <div className="flex items-center gap-2">
                    <Text strong>#{selectedProductionPlan?.id || selectedPlan?.id}</Text>
                    <Text>
                      {(selectedProductionPlan || selectedPlan)?.sourceproductname || 
                       (selectedProductionPlan || selectedPlan)?.alloyName} → 
                      {(selectedProductionPlan || selectedPlan)?.targetproductname ||
                ...
                <Descriptions.Item label="Quantity">
                  <Text strong>{form.getFieldValue('quantity')?.toLocaleString()} units</Text>
                  {showReasonField && (
                ...
                <Descriptions.Item label="Allocation">
                  <Progress
                    percent={Math.round(
                      ((form.getFieldValue('quantity') || 0) / allocationInfo.totalQuantity) * 100
                    )}
                    size="small"
                    strokeColor="#1890ff"
                  />
                ...
                <Descriptions.Item label="Total Steps">
                  <Badge 
                    count={(selectedProductionPlan || selectedPlan)?.workflowInfo?.totalSteps ?? (selectedProductionPlan || selectedPlan)?.stepCount ?? 11}
              ...
              <div className="mt-4">
                ...
                <div className="max-h-40 overflow-y-auto">
                  {(stepMode === 'preset' ? selectedPreset?.steps || [] : customSteps)
                    .map((step, index) => (
                      <Tag 
                        key={step.id} 
                        ...
                        className="mb-2 mr-2"
                      >
                        {index + 1}. {step.name || step.stepName}
                      </Tag>
                    ))}
                </div>
              </div>
