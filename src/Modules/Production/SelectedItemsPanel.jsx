...
// Quantity input with local state to prevent focus loss
const QuantityInput = ({ planId, quantity, onUpdate }) => {
  ...
  const handleBlur = () => {
    if (localValue === '') {
      onUpdate(planId, 'quantity', '')
      return
    }
    const numValue = parseInt(localValue)
    if (!isNaN(numValue) && numValue > 0) {
      onUpdate(planId, 'quantity', numValue)
    } else {
...
const SelectedItemsPanel = ({
      ...
      <div style={style} className='px-3 py-2'>
        ...
        >
          ...
          <div className='flex items-center gap-2 mb-2'>
            <Tag
              color={
                totalStock === 0 ? 'red' : totalStock < 10 ? 'orange' : 'green'
              }
            >
  ...
  if (isCollapsed) {
    return (
      <div
        className='w-12 h-full bg-white border-l flex flex-col items-center py-4 fixed right-0 top-0 z-10'
        style={{ height: '100vh' }}
      >
        ...
        <div className='flex flex-col items-center gap-2'>
          ...
          {stats.notConfigured > 0 && (
            <Tooltip
              title={`${stats.notConfigured} need configuration`}
              placement='left'
            >
  ...
  return (
    <div
      className='w-[400px] h-full bg-white border-l flex flex-col fixed right-0 top-0 z-10 shadow-lg'
      style={{ height: '100vh' }}
    >
      {/* Header */}
      <div className='px-4 py-3 border-b bg-gray-50'>
        <div className='flex items-center justify-between mb-2'>
          ...
          <Space>
            {stats.total > 0 && (
              <Button size='small' danger onClick={onRemoveAll}>
                Clear All
              </Button>
            )}
      ...
      <div className='flex-1 overflow-hidden'>
        {selectedItems.length === 0 ? (
          <Empty description='No items selected' className='mt-8' />
