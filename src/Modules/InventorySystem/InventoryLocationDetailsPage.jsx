...
const InventoryLocationDetailsPage = () => {
  ...
  const [storageAreasModalVisible, setStorageAreasModalVisible] = useState(false);
  ...
  const fetchInventory = useCallback(async () => {
    ...
    try {
      const response = await client.get(`/inventory/internal/locations/${locationId}/inventory`);
  ...
  const inventoryStats = {
    ...
    totalReserved: inventory.reduce((sum, item) => sum + (item.reservedQuantity || 0), 0),
    totalAvailable: inventory.reduce((sum, item) => sum + (item.availableQuantity || 0), 0),
    uniqueProducts: new Set(inventory.map(item => `${item.productType}-${item.productId}`)).size,
  ...
  const movementsColumns = [
    ...
    },
    {
      ...
      render: (record) => (
        <div>
          <Tag color={record.productType === 'alloy' ? 'blue' : 'green'} style={{ marginRight: '4px' }}>
            {record.productType?.toUpperCase()}
          </Tag>
    ...
    return (
      <div style={{ padding: '24px' }}>
        ...
        <div style={{ textAlign: 'center', marginTop: '16px' }}>
          <Button type="primary" onClick={() => navigate('/inventory-locations')}>
            Back to Locations
          </Button>
        </div>
      </div>
      ...
      children: (
        <div>
          ...
          <Row gutter={16} style={{ marginBottom: '16px' }}>
            <Col span={8}>
              <Search
                placeholder="Search by product ID, type, area..."
                allowClear
                value={searchText}
          ...
          <Table
            ...
            locale={{
              emptyText: <Empty description="No inventory items in this location" />
            }}
            size="middle"
          />
        </div>
      ...
      children: (
        <div>
          ...
          <Table
            ...
            locale={{
              emptyText: <Empty description="No movements recorded for this location" />
            }}
            size="middle"
          />
        </div>
      ...
      <Card size="small" style={{ marginBottom: '24px' }}>
        <Descriptions column={4} size="small">
          ...
          <Descriptions.Item label="Utilization">
            <Progress
              percent={inventoryStats.totalItems > 0 ? Math.min(100, Math.round((inventoryStats.totalItems / 1000) * 100)) : 0}
              size="small"
              style={{ width: '120px' }}
            />
          </Descriptions.Item>
